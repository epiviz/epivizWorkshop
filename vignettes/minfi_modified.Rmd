...
epivizEnv <- EpivizEnvironment(chr="chr11", start= 108000000, end = 131000000)
...
Epiviz environment and navigation components enable brushing across different charts. In addition, 
these components also understand genomic context. We can use it to obtain information about 
...
to see all the charts inside an environment at any time 
...
TODO: update settings and colors
...
In the epiviz environment `epivizEnv`, you can use the hovering feature
...
This time, we are plotting things a bit differently. when we create an `EpivizChart` object, it does two things: first, it registers the data object we pass to it as a data source in epiviz, and then it adds a chart with the **default** chart type defined for that data type. 
...
(or) Second, we can create a new `EpivizNavigation` element, and assign it to the genomic location of the TRIM29 gene.

```{r}
epivizNav = EpivizNavigation("chr11", 119500000, 120100000, parent=epivizEnv)

# pvals_track <- EpivizChart(dmp_gr, parent=epivizNav, chart="StackedLineTrack", columns="pval", type="bp")

measurements <- beta_track$get_measurements()
beta_nav_track <- EpivizChart(measurements=measurements, parent= epivizNav, chart="LineTrack")
# pvals_track
# epivizNav
...
Let's remove all the charts added to the environment.
