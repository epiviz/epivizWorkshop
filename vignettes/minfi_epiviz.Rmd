...
genes_track <- epivizChart(Homo.sapiens, chr="chr11", start= 118000000, end = 121000000, parent=epivizEnv)
...
gm_set_breast <- gm_set[,which(gm_set$Tissue=="breast")]
breast_betas <- getBeta(gm_set_breast)
pd <- pData(gm_set_breast)
sample_indices <- split(seq(len=nrow(pd)), pd$Status)

mean_meth_breast <- sapply(sample_indices, function(ind) rowMeans(breast_betas[,ind]))

dmp_gr$columnM = mean_meth_breast[, "cancer"] - mean_meth_breast[, "normal"]
dmp_gr$columnA = 0.5 * (mean_meth_breast[, "cancer"] + mean_meth_breast[, "normal"])


cpg_gr <- granges(gm_set)
mcols(cpg_gr) <- mean_betas

ma_plot <- epivizChart(dmp_gr, parent=epivizEnv, chart="ScatterPlot", columns=c("columnA", "columnM"), type="bp")
ma_plot
```
...
```{r, eval=FALSE}
epivizEnv$navigate("chr11", 119500000, 120100000)
epivizEnv
```
...
```{r}
epivizNav <- epivizNav("chr11", 119500000, 120100000, parent=epivizEnv)

# needs epivizrChart branch - nav_branch to work.
# copy charts from env
# charts <- epivizEnv$get_charts() 
# epivizNav$clone_charts(charts) 

# or using init_region
# epivizEnv$init_region(chr="chr11", start=119500000, end=120100000)
...
Let's start by adding gene annotation track, CpG islands and heatmap for breat(tumor and cancer), lung and colon tissues from the gene expression barcode datsest, similar to what we did at the beginning of this vignette
